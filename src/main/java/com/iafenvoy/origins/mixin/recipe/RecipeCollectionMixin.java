...
public abstract class RecipeCollectionMixin {
    ...
    private boolean accountForPowerRecipes(boolean original, StackedContents recipeFinder, int gridWidth, int gridHeight, RecipeBook recipeBook, @Local RecipeHolder<?> recipeEntry) {
        return original && recipeEntry.value() instanceof PowerCraftingRecipe pcr && recipeBook instanceof PowerCraftingObject pco && pco.origins$getPlayer().flatMap(OriginDataHolder::optional).map(h -> h.hasActivePower(pcr.powerId(), RecipePower.class)).orElse(true);
    }
}
