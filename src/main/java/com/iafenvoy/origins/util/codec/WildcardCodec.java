...
public enum WildcardCodec implements Codec<Identifier> {
    INSTANCE;
        ...
        return ops.getStringValue(input).flatMap(s -> {
            if (s.contains("*")) {
                if (ops instanceof ResourceLoadingOps keyGetter) {
                    ...
                    if (resourceKey != null) {
                        Identifier key = resourceKey.identifier();
                        String[] split = s.split(":");
                        if (split.length == 2) {
                            Identifier id = Identifier.tryBuild(split[0].replaceAll("\\*", key.getNamespace()), split[1].replaceAll("\\*", key.getPath()));
            ...
            } else return Identifier.CODEC.decode(ops, input);
        });
    }

    @Override
    public <T> DataResult<T> encode(Identifier input, DynamicOps<T> ops, T prefix) {
        return Identifier.CODEC.encode(input, ops, prefix);
    }
}
