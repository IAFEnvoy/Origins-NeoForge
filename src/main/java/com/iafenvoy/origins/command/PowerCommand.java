...
public final class PowerCommand {
    ...
    private static CompletableFuture<Suggestions> suggestPowers(final CommandContext<CommandSourceStack> context, final SuggestionsBuilder builder) throws CommandSyntaxException {
        ...
        Stream<String> stream = sources.stream().map(Holder::unwrapKey).filter(Optional::isPresent).map(Optional::get).map(ResourceKey::identifier).map(Identifier::toString);
    ...
    private static CompletableFuture<Suggestions> suggestAllSources(final CommandContext<CommandSourceStack> context, final SuggestionsBuilder builder) throws CommandSyntaxException {
        ServerPlayer player = EntityArgument.getPlayer(context, "target");
        Set<Identifier> sources = new HashSet<>(OriginDataHolder.get(player).getData().getPowers().keySet());
        sources.add(OriginDataHolder.DEFAULT_SOURCE);
        return SharedSuggestionProvider.suggest(sources.stream().map(Identifier::toString), builder);
    }
    ...
    private static CompletableFuture<Suggestions> suggestSources(final CommandContext<CommandSourceStack> context, final SuggestionsBuilder builder) throws CommandSyntaxException {
        ...
        Stream<String> stream = OriginDataHolder.get(player).getData().getPowers().entries().stream().filter(x -> x.getValue().value() == power.value()).map(Map.Entry::getKey).map(Identifier::toString);
        return SharedSuggestionProvider.suggest(stream, builder);
    }
    ...
    private static int grantFromSource(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        return grant(context, IdentifierArgument.getId(context, "source"));
    }

    private static int grant(CommandContext<CommandSourceStack> context, Identifier source) throws CommandSyntaxException {
    ...
    private static int revokeFromSource(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        return revoke(context, IdentifierArgument.getId(context, "source"));
    }

    private static int revoke(CommandContext<CommandSourceStack> context, Identifier source) throws CommandSyntaxException {
        ServerPlayer target = EntityArgument.getPlayer(context, "target");
        Holder<Power> power = ResourceArgument.getResource(context, "power", PowerRegistries.POWER_KEY);
    ...
    private static int clear(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        OriginDataHolder holder = OriginDataHolder.get(target);
        // 收集所有条目以避免并发修改
        List<Map.Entry<Identifier, Holder<Power>>> entries = new ArrayList<>(holder.getData().getPowers().entries());
        for (Map.Entry<Identifier, Holder<Power>> e : entries) holder.revokePower(e.getKey(), e.getValue());
        holder.sync();
    ...
    private static int has(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        OriginDataHolder holder = OriginDataHolder.get(target);
        try {
            Identifier source = IdentifierArgument.getId(context, "source");
            if (holder.getEntityPowers().containsEntry(source, power))
    ...
    private static int sources(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        Holder<Power> power = ResourceArgument.getResource(context, "power", PowerRegistries.POWER_KEY);
        OriginDataHolder holder = OriginDataHolder.get(target);
        String list = holder.getData().getPowers().entries().stream().filter(e -> e.getValue().equals(power)).map(Map.Entry::getKey).map(Identifier::toString).reduce((a, b) -> a + ", " + b).orElse("(none)");
