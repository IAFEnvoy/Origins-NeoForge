...
public record InventoryCondition(InventoryUtil.ProcessMode processMode, ItemCondition itemCondition, IntList slot,
                                 Optional<Holder<Power>> power, Comparison comparison) implements EntityCondition {
    ...
    public boolean test(@NotNull Entity entity) {
        ...
        if (this.power.isEmpty())
            ...
            if (!(targetPower.value() instanceof InventoryPower inventoryPower)) return false;
            matches += InventoryUtil.checkInventory(this.itemCondition, this.slot, entity, inventoryPower, this.processMode.getProcessor());
        }
