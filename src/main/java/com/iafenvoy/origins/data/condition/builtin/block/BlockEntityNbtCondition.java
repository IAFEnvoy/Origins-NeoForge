...
public record BlockEntityNbtCondition(CompoundTag nbt) implements BlockCondition {
    public static final MapCodec<BlockEntityNbtCondition> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            CompoundTag.CODEC.fieldOf("nbt").forGetter(BlockEntityNbtCondition::nbt)
    ).apply(i, BlockEntityNbtCondition::new));
