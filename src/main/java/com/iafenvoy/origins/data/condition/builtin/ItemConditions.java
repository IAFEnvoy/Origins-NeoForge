...
public final class ItemConditions {
    ...
    public static final DeferredHolder<MapCodec<? extends ItemCondition>, MapCodec<? extends ItemCondition>> FOOD = REGISTRY.register("food", () -> createItem((level, stack) -> stack.get(net.minecraft.core.component.DataComponents.FOOD) != null));
    ...
    public static final DeferredHolder<MapCodec<? extends ItemCondition>, MapCodec<? extends ItemCondition>> SMELTABLE = REGISTRY.register("smeltable", () -> createItem((level, stack) -> level instanceof ServerLevel serverLevel && serverLevel.getServer().getRecipeManager().getRecipeFor(RecipeType.SMELTING, new SingleRecipeInput(stack), level).isPresent()));
