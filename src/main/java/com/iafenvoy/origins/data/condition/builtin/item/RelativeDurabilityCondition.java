...
public record RelativeDurabilityCondition(Comparison comparison) implements ItemCondition {
    public static final MapCodec<RelativeDurabilityCondition> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            Comparison.CODEC.forGetter(RelativeDurabilityCondition::comparison)
    ...
    public boolean test(@NotNull Level level, @NotNull ItemStack stack) {
        return stack.isDamageableItem() && this.comparison.compare(Math.abs((float) (stack.getMaxDamage() - stack.getDamageValue())) / (float) stack.getMaxDamage());
    }
}
