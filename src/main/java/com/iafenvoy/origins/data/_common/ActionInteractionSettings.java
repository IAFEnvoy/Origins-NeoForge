...
public record ActionInteractionSettings(ItemCondition itemCondition, List<InteractionHand> hands,
                                        Optional<ItemStack> resultStack, ItemAction heldItemAction,
                                        ItemAction resultItemAction, InteractionResult actionResult) {
    public static final MapCodec<ActionInteractionSettings> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            ItemCondition.optionalCodec("item_condition").forGetter(ActionInteractionSettings::itemCondition),
            ExtraEnumCodecs.HAND.listOf().optionalFieldOf("hands", List.of(InteractionHand.values())).forGetter(ActionInteractionSettings::hands),
            ItemStack.CODEC.optionalFieldOf("result_stack").forGetter(ActionInteractionSettings::resultStack),
            ItemAction.optionalCodec("held_item_action").forGetter(ActionInteractionSettings::heldItemAction),
            ItemAction.optionalCodec("result_item_action").forGetter(ActionInteractionSettings::resultItemAction),
            ExtraEnumCodecs.INTERACTION_RESULT.optionalFieldOf("action_result", InteractionResult.SUCCESS).forGetter(ActionInteractionSettings::actionResult)
    ).apply(i, ActionInteractionSettings::new));
