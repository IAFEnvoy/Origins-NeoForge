...
public class ModifyDamageDealtPower extends Power implements ModifierPowerHelper {
    ...
    public static void onDamage(LivingDamageEvent.Pre event) {
        ...
        if (source == null || event.getSource().is(DamageTypeTags.IS_PROJECTILE))// 弹射物由ModifyProjectileDamagePower处理
            return;
        OriginDataHolder holder = OriginDataHolder.get(source);
        holder.streamActivePowers(ModifyDamageDealtPower.class).forEach(power -> {
            float baseValue = event.getContainer().getNewDamage();
            ...
            if (power.damageCondition.test(s, baseValue) && power.targetCondition.test(target) && power.biEntityCondition.test(source, target)) {
                event.getContainer().setNewDamage(power.modify(holder, baseValue));
