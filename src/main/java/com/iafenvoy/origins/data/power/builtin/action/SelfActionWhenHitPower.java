...
public class SelfActionWhenHitPower extends HasCooldownPower {
    public static final MapCodec<SelfActionWhenHitPower> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
    ...
    public SelfActionWhenHitPower(BaseSettings settings, CooldownSettings cooldown, EntityAction entityAction, DamageCondition damageCondition) {
    ...
    public static void onHit(LivingDamageEvent.Post event) {
        ...
        holder.getHelper().execute(SelfActionWhenHitPower.class,
                p -> p.damageCondition.test(event.getSource(), event.getInflictedDamage()),
