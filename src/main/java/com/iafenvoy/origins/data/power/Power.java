...
public abstract class Power {
    ...
    public void collectBadges(ImmutableSet.Builder<Badge> builder) {
        this.settings.badges().forEach(badge -> builder.add(badge.map(Holder::value, Function.identity())));
    }
    ...
    @Comment("tick() 的时间间隔，与 active() 和 inactive() 相关")
    ...
    //修复::应使用 PowerHolder 替代
    public Identifier getId(RegistryAccess access) {
        Registry<Power> registry = access.lookupOrThrow(PowerRegistries.POWER_KEY);
    ...
    public MutableComponent getName(Identifier id) {
    ...
    public record BaseSettings(Optional<Component> name, Optional<Component> description, boolean hidden,
                               EntityCondition condition, int loadingPriority, List<Either<Holder<Badge>, Badge>> badges) {
        public static final MapCodec<BaseSettings> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
                ...
                Codec.either(Badge.CODEC, Badge.DIRECT_CODEC).listOf().optionalFieldOf("badges", List.of()).forGetter(BaseSettings::badges)
