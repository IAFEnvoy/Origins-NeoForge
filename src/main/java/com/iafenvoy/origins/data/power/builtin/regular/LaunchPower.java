    ...
    public void toggle(@NotNull OriginDataHolder holder, String key) {
        ...
        this.getCooldownComponent(holder).useIfReady(() -> {
                ...
                if (entity.level() instanceof ServerLevel serverLevel) {
                    ...
                    this.sound.ifPresent(s -> serverLevel.playSound(null, entity.getX(), entity.getY(), entity.getZ(), s, SoundSource.NEUTRAL, 0.5F, 0.4F / (entity.level().getRandom().nextFloat() * 0.4F + 0.8F)));
                    for (int i = 0; i < 4; ++i)
                        serverLevel.sendParticles(ParticleTypes.CLOUD, entity.getX(), entity.getRandomY(), entity.getZ(), 8, entity.level().getRandom().nextGaussian(), 0.0D, entity.level().getRandom().nextGaussian(), 0.5);
                }
        });
    }
}
