...
public class ToggleComponent extends PowerComponent {
    ...
    public void sendMessage(OriginDataHolder holder, String key) {
        if (holder.getEntity() instanceof Player player)
            if (player instanceof net.minecraft.server.level.ServerPlayer sp) sp.sendSystemMessage(Component.translatable(key).append(": ").append(this.active ? Component.literal("ON").withColor(0xFF00FF00) : Component.literal("OFF").withColor(0xFFFF0000)), true);
    }
