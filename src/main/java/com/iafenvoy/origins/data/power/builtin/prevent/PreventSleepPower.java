        ...
        OriginDataHolder.get(event.getEntity()).streamActivePowers(PreventSleepPower.class).forEach(x -> {
            if (x.blockCondition.test(event.getLevel(), event.getPos())) {
                if (x.shouldSetSpawnPoint())
                    if (event.getEntity() instanceof net.minecraft.server.level.ServerPlayer sp) sp.setRespawnPosition(new net.minecraft.server.level.ServerPlayer.RespawnConfig(net.minecraft.world.level.storage.LevelData.RespawnData.of(event.getLevel().dimension(), event.getPos(), 0.0F, 0.0F), false), true);
                event.setProblem(Player.BedSleepingProblem.OTHER_PROBLEM);
                if (event.getEntity() instanceof net.minecraft.server.level.ServerPlayer sp) sp.sendSystemMessage(x.message, true);
            }
        });
    }
}
