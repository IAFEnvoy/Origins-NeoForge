...
public class EdibleItemPower extends Power implements Prioritized {
    public static final MapCodec<EdibleItemPower> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            ...
            ExtraEnumCodecs.USE_ANIM.optionalFieldOf("consume_animation", ItemUseAnimation.EAT).forGetter(EdibleItemPower::getConsumeAnimation),
            BuiltInRegistries.SOUND_EVENT.byNameCodec().optionalFieldOf("consume_sound", SoundEvents.GENERIC_EAT.value()).forGetter(EdibleItemPower::getConsumeSound),
    ...
    public EdibleItemPower(BaseSettings settings, EntityAction entityAction, ItemAction itemAction, ItemAction resultItemAction, ItemCondition itemCondition, FoodProperties foodProperties, Optional<ItemStack> resultStack, ItemUseAnimation consumeAnimation, SoundEvent consumeSound, List<Modifier> consumingTimeModifier, int priority) {
        super(settings);
