...
public final class OriginsRegistries {
    ...
    public static void fillParentAfterLoad(TagsUpdatedEvent event) {
        Registry<Power> registry = event.getRegistries().lookupOrThrow(PowerRegistries.POWER_KEY);
        //修复::需要处理递归情况？
        for (Holder.Reference<Power> p : registry.listElements().toList()) {
    ...
    public static void afterBuiltinLoaded(FMLLoadCompleteEvent event) {
        if (!FMLEnvironment.isProduction() || OriginsConfig.INSTANCE.debug.builtinRegistries.getValue()) {
            Origins.LOGGER.info("Origins builtin registries loaded, print object counts.");
            for (Registry<?> registry : BUILTIN_REGISTRIES)
                Origins.LOGGER.info("Builtin Registry: {}, objects count: {}", registry.key().identifier(), registry.stream().count());
        }
    }

    @SubscribeEvent
    public static void afterDynamicLoaded(TagsUpdatedEvent event) {
        if (!FMLEnvironment.isProduction() || OriginsConfig.INSTANCE.debug.dynamicRegistries.getValue()) {
            RegistryAccess access = event.getRegistries();
            ...
            for (ResourceKey<? extends Registry<?>> key : DYNAMIC_REGISTRIES)
                Origins.LOGGER.info("Dynamic Registry: {}, objects count: {}", key.identifier(), access.lookupOrThrow(key).stream().count());
        }
    }
}
