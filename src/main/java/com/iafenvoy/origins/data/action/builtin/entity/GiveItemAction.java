...
public record GiveItemAction(ItemStackReference stack, ItemAction itemAction,
    ...
    public static final MapCodec<GiveItemAction> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            ItemStackReference.CODEC.fieldOf("stack").forGetter(GiveItemAction::stack),
    ...
    public void execute(@NotNull Entity source) {
        if (source instanceof Player player) {
            ItemStack stack = this.itemAction.execute(player.level(), source, this.stack.create());
