...
public record ItemIfElseListAction(List<ConditionedActionHolder> actions) implements ItemAction {
    public static final MapCodec<ItemIfElseListAction> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            ConditionedActionHolder.CODEC.listOf().fieldOf("actions").forGetter(ItemIfElseListAction::actions)
    ).apply(i, ItemIfElseListAction::new));
