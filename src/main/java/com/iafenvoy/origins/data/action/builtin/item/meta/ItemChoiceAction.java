...
public record ItemChoiceAction(List<WeightedActionHolder> actions) implements ItemAction {
    public static final MapCodec<ItemChoiceAction> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            WeightedActionHolder.CODEC.listOf().fieldOf("actions").forGetter(ItemChoiceAction::actions)
    ).apply(i, ItemChoiceAction::new));
